...
public class TVSeries extends Production {
    ...
    public String toString() {
        return "\n//////////////////////////////////////////"
            + this.getTitle() + separator
            + this.getDescription() + separator
            + this.getReleaseDate().getYear() + separator + this.getReleaseDate().getMonthValue() + separator + this.getReleaseDate().getDayOfMonth() + separator 
            + "\n//////////////////////////////////////////";
    };
    ...
    public void calculateEpisodeNum() { // For every episode, and each role in a cast, if a person is in an episode, add appearance of that actor in a list
        ...
        for (Episode ep : episodes) {
            ArrayList<Person> peoplePlayed = new ArrayList<>();
            for (Role role : ep.getRoles()) {
                Person actor = role.getActor();
                if (!peoplePlayed.contains(actor)) {
                    actor.addAppearance();     
                    peoplePlayed.add(actor);
                }
            }
        }
    ...
    public TVSeries() {
        super("Unknown Title", "Unknown Description", 0, LocalDate.now(), null, new ArrayList<>());
    }
